// Package server encodes and validates the wire format for chat messages.
//
// This file exists to keep one invariant in one place: the output of
// [normalizeMessage] must be byte-for-byte what [encoding/json] would produce
// for the same [Message]. The fast paths below only pay off because they are
// exactly equivalent to the standard library, so any change here must keep
// TestNormalizeMessageQuick and TestAppendJSONStringExhaustiveRunes green.
package server
